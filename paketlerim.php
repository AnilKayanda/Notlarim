...
if (isset($_SESSION['Eposta'])) {
    ...
    $sql = "SELECT DISTINCT GROUP_CONCAT(dersler.ID) AS DersIDList, GROUP_CONCAT(dersler.Dersler) AS Dersler 
            FROM satinalinanlar
            JOIN dersler ON FIND_IN_SET(dersler.ID, satinalinanlar.Yayinlar)
            WHERE satinalinanlar.ID = $userID
            GROUP BY satinalinanlar.ID";
...
<body>
    ...
    <section>
        ...
        foreach ($userPackages as $ders) {
            ...
            echo '<a target="_blank" href="notlar.php?id=' . $ders['DersID'] . '" onclick="updateIframeSrc(' . $ders['DersID'] . ')">';
            echo '<div class="news">';
